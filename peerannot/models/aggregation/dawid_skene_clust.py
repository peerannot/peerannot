...
class DawidSkeneClust(CrowdModel):
    ...
    def initialize_parameter(self, x, K, L, random=True, delta=1e-10):
        ...
        if random:
            theta = np.clip(
                np.einsum(
                    "ki->ik",
                    np.einsum("ijk->ki", x) / np.einsum("ijk->i", x),
                )
            ...
            theta = np.einsum(
                "ki->ik",
                np.einsum("ik->ki", theta) / np.sum(theta, axis=1),
            )
        else:
            theta = np.einsum(
                "ki->ik",
    ...
    def variational_update(
        ...
        theta = np.einsum(
            "ki->ik",
            theta_prime.T / np.sum(theta_prime.T, axis=0),
        )
    ...
    def hyper_parameter_update(self, x, theta, phi):
        ...
        lambda_ = np.einsum(
            "mlk->lkm",
            lambda_prime / np.sum(lambda_prime, axis=0),
        )
